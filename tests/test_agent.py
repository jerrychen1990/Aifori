...
class TestAgent(unittest.TestCase):
    ...
    def test_agent_chat(self):
        ...
        # 测试问答
        message: UserMessage = UserMessage(content="你好，我最近心情不好，不知道该怎么办", user_id=self.user_id)
        resp = self.agent.chat(message, max_tokens=128)
        logger.info(resp)

        # 测试流式
        message: UserMessage = UserMessage(content="能给我推荐三首歌么", user_id=self.user_id)
        ...
        # 测试多轮
        message: UserMessage = UserMessage(content="详细介绍一下第三首", user_id=self.user_id)
        resp = self.agent.chat(message, max_tokens=64)
        logger.info(resp)

    def test_lingxin_agent(self):
        lingxin_agent = create_assistant(id="lingxin_agent", name="lingxin_agent", desc=DEFAULT_AI_DESC, model="emohaa", exists_ok=True)
